function [U, W, stats] = rtrmc_mod(problem, problem_test, opts, U0)
...
% See also: buildproblem initialguess
    ...
    if ~isempty(problem_test)
        X_test = problem_test.X;
        C_test = problem_test.C;
        I_test = problem_test.I;
        J_test = problem_test.J;
    end
    ...
    if ~isempty(problem_test)    
        opts.statsfun = @mc_mystatsfun;
    end
    
  
    % Do the magic.
    if strcmpi('rtr', opts.method)
        optproblem.ncostterms = n;
        [U, bestcost, stats] = subsampled_rtr(optproblem, U0, opts); %#ok<ASGLU>
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function [grad, store] = gradient(U, store)
        ...
        if ~isfield(store, 'grad')
            ...
            store.grad = (multsparsefull(RU, W.', mask) + U*sqlaWWt)/n;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function stats = mc_mystatsfun(problem, U, stats)
            ...
            W_test = lsqfit(problem_test, U);
            
            UW = spmaskmult(U, W_test, I_test, J_test);
            
            f = ( .5*sum((C_test.*(UW-X_test)).^2)    ...
                        + .5*lambda.^2*sum(W(:).^2) ...
                        - .5*lambda.^2*sum(UW.^2)   ) / n;
