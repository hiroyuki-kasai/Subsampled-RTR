...
% Change log: 
%
%   May 18, 2018 (HK: Hiroyuki Kasai):
%       Adds a mechanism to measure oracle calls.
    ...
    oraclecalls = problem.ncostterms;   % HK         
    ...
    for epoch = 1 : options.maxepoch
        ...
        for inneriter = 1 : totalbatches
            ...
            % Update oracle calls
            oraclecalls = oraclecalls + batchsize;  % for partialgrad at x.     % HK          
            ...
            elseif strcmp(options.stepsize_type, 'hybrid')
                ...
                if epoch < 3 % Decay stepsize only for the initial few epochs.                    
                    stepsize = stepsize0 / (1  + stepsize0 * options.stepsize_lambda * iter); % Decay with O(1/iter).
                end
            ...
            if strcmp(options.update_type, 'svrg')
                ...
                % update Oracle calls 
                oraclecalls = oraclecalls + batchsize;  % for partialgrad at x0    % HK
                oraclecalls = oraclecalls + 1;          % for vector transport     % HK             
        ...
        oraclecalls = oraclecalls + problem.ncostterms;       % HK     
    ...
    function stats = savestats()
        ...
        stats.oraclecalls = oraclecalls;   % HK         
        if epoch == 0
            stats.time = 0;
        else
