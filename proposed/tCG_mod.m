...
%                  = tCG_mod(problem, x, grad, eta, Delta, options, storedb, key)
function [eta, Heta, inner_it, stop_tCG] ...
                 = tCG_mod(problem, x, grad, eta, Delta, subsamphess_idx_batch, samp_hess_p, options, storedb, key)             
...
for j = 1 : options.maxinner
    ...
    if length(subsamphess_idx_batch) == problem.ncostterms
        Hdelta = getHessian(problem, x, delta, storedb, key);  
    else
        Hdelta = getPartialHessian(problem, x, delta, subsamphess_idx_batch, samp_hess_p, storedb, key); 
    end
