function [x, cost, info, options] = trustregions_mod(problem, x, options)
...
tcg_stop_reason = {'negative curvature',...
                   ...
                   'reached target residual-kappa (lin)',...
...
if ~isfield(options,'Delta0')
    ...
    options.Delta0 = options.Delta_bar / 2;    
end
...
% decide samp_hess_mode : 
mode = 'R-TR';
if strcmp(options.samp_hess_scheme, 'fix')
    %
elseif strcmp(options.samp_hess_scheme, 'lin')   
    %   
elseif strcmp(options.samp_hess_scheme, 'adp')   
    %      
elseif strcmp(options.samp_hess_scheme, 'exp')
    %hess_exp_growth_constant = (problem.ncostterms-options.samp_hess_init_size)^(1/(options.maxiter));
    hess_exp_growth_constant = (problem.ncostterms-options.samp_hess_init_size)^(1/15);
end 

if strcmp(options.samp_grad_scheme, 'fix')
    %
elseif strcmp(options.samp_grad_scheme, 'lin')   
    %
elseif strcmp(options.samp_grad_scheme, 'adp')   
    %    
elseif strcmp(options.samp_grad_scheme, 'exp')
    %grad_exp_growth_constant = (problem.ncostterms-options.samp_grad_init_size)^(1/(options.maxiter));
    grad_exp_growth_constant = (problem.ncostterms-options.samp_grad_init_size)^(1/15);    
end 
...
if strcmp(options.samp_grad_scheme, 'full')
    ...
    %samp_grad_idx = randi(problem.ncostterms, options.samp_grad_init_size, 1); 
    ...
    % Oracle calls counter. 
    oraclecalls = options.samp_grad_init_size;         
end
...
stats = savestats(problem, x, storedb, key, options, k, fx, norm_grad, Delta, oraclecalls, ticstart); 
info(1) = stats;
...
while true
    ...
    if strcmp(options.samp_hess_scheme, 'exp')
        samp_hess_size = fix(options.samp_hess_init_size + hess_exp_growth_constant^(k+1))+1;
    elseif strcmp(options.samp_hess_scheme, 'lin')
        samp_hess_size = options.samp_hess_init_size * (k+1);
    elseif strcmp(options.samp_hess_scheme, 'fix')     
    ...
    elseif strcmp(options.samp_hess_scheme, 'adp') 
        ...
        if k == 0
            ...
            if k == 1
                c_Hessian = (options.samp_hess_init_size * eta_norm.^2)/log(problem.d);
            end
            if accept == false
                samp_hess_size = fix(samp_hess_size * options.unsuccessful_sample_scaling);
            else
                samp_hess_size = fix(max([(c_Hessian*log(problem.d)/(eta_norm.^2) * options.sample_scaling_Hessian), options.samp_hess_init_size]));            
            end
        end
...
%                 tCG(problem, x, fgradx, eta, Delta, options, storedb, key);
    [eta, Heta, numit, stop_inner] = ...
                tCG_mod(problem, x, fgradx, eta, Delta, samp_hess_idx, samp_hess_p, options, storedb, key);
    srstr = tcg_stop_reason{stop_inner};
    
    eta_norm = problem.M.norm(x, eta);
    
    
    oraclecalls = oraclecalls + numit * samp_hess_size;  
    ...
    if options.use_default_update_alg
        ...
        if strcmp(options.samp_grad_scheme, 'exp')
            samp_grad_size = fix(options.samp_grad_init_size + grad_exp_growth_constant^(k+1))+1;
        elseif strcmp(options.samp_grad_scheme, 'lin')
            samp_grad_size = options.samp_grad_init_size * (k+1);
        elseif strcmp(options.samp_grad_scheme, 'fix')     
        ...
        elseif strcmp(options.samp_grad_scheme, 'adp')  
            ...
            if k == 0
                ...
                % adjust sampling constant c such that the first step would have given a sample size of initial_sample_size
                if k == 1
                    c_gradient = (options.samp_grad_init_size * eta_norm.^4)/log(problem.d);
                end
                if accept == false
                    samp_grad_size = fix(samp_grad_size * options.unsuccessful_sample_scaling);
                else
                    samp_grad_size = fix(max([(c_gradient*log(problem.d)/(eta_norm.^4)*options.sample_scaling_gradient), options.samp_grad_init_size]));
                end
            end        
        ...
        % decide sub-sample indices for gradient    
        if samp_grad_size ~= problem.ncostterms 
            % Draw the samples with replacement.
            %samp_grad_size = problem.ncostterms-1;
            samp_grad_idx = randsample(problem.ncostterms, samp_grad_size); 
            %samp_grad_idx = randi(problem.ncostterms, samp_grad_size, 1); : does not work properly   
        else
            samp_grad_idx = 1:problem.ncostterms;
        end    
    ...
    else % This is original update alg
...
%                       rhoden, accept, numit, norm_eta, used_cauchy);
    stats = savestats(problem, x, storedb, key, options, k, fx, ...
                      norm_grad, Delta, oraclecalls, ticstart, info, rho, rhonum, ...
                      rhoden, accept, numit, norm_eta, used_cauchy);
    ...
    if options.verbosity == 2,
        ...
        'f: %+.16e   |grad|: %e   %s\n'], ...
        mode, options.samp_hess_scheme, options.samp_grad_scheme, accstr,trstr,k,numit,fx,norm_grad,srstr);
    elseif options.verbosity > 2,
